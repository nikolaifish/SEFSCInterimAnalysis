...
rm(list=ls())
knitr::opts_chunk$set(comment=NA,echo=FALSE,message=FALSE,warning=FALSE,fig.width=9,fig.height=11)
...
# ts_reldiff()
# Compute relative absolute differences in time series values
ts_reldiff <- function(x,denom_type="x2", # "rollmean" can also be used for the denominator for scaling the diffs
                       lag=1,type="absreldiff",
                       rollmean_args=list(k=11,fill="extend")
                       ){
  Yrs <- c(1,length(x))
  y1 <- Yrs[1]:(Yrs[2] - lag)
  y2 <- (Yrs[1] + lag):Yrs[2]
  x1 <- x[y1]
  x2 <- x[y2]
  rollmeanx2 <- do.call(rollmean,c(list(x=x),rollmean_args))[y2]

  diff <-   x1-x2
  denom <- switch(denom_type,
                  x2=x2,
                  rollmean=  rollmeanx2
                  )
  reldiff <- diff/denom
  absreldiff <- (reldiff^2)^0.5
  out <- switch(type,
                diff=diff,
                reldiff=reldiff,
...
boxplot_outline <- FALSE # Should outliers be plotted in boxplots?
boxplot_range <- 1.5 # Set range of whiskers in boxplots. A value of zero plots whiskers to data extremes.
...
# Sets of scenarios
sN1 <- "base"
sN2 <-  c("base","ucvhi","ucvlo","ubias","tachi","taclo")
sN3 <-  c("base","rc","minerr","minrecdev","recns","vtiv","vdome")
sN4 <-  c("base","noempind","catcvlo","nolag","perfobs","dep","lf")
sN5 <-  c("base","lhset","Mset","steepset","constM","epiM")


sN_all <- c("sN1","sN2","sN3","sN4","sN5")
...
# All scenarios
sN <- unique(as.character(unlist(sapply(sN_run,get))))
...
MPNamesSub_userB <- c("SCA_1","SCA_5","pMP_5","iMP_avg_5","iMP_buffer_5")
MPNamesSub_userB_legtext <- MPNamesAll_key[MPNamesSub_userB]

MPNamesSub_userA <- c("SCA_1","SCA_10","pMP_10","iMP_avg_10","iMP_buffer_10")
MPNamesSub_userA_legtext <- MPNamesAll_key[MPNamesSub_userA]

MPNamesAll_abb <- local({
  a <- MPNamesAll
  a <- gsub("iMP_buffer","Bfr I",a)
  a <- gsub("iMP_avg","Avg I",a)
  a <- gsub("pMP","Proj",a)
  a <- gsub("iMP_","I",a)
  a <- gsub("_([0-9]+)$"," (\\1)",a)
  a
...
YieldLT <- function(...,Ref=1,Yrs=-10){Yield(...,Ref=Ref,Yrs=Yrs)}
...
mgp=c(1.5,0.2,0),
cex.axis=1.2,
plot_format="landscape", # "portrait" or "landscape". Are species plots next to each other or stacked vertically?
...
if(plot_type=="addind"){plot_format <- "portrait"}
if(plot_format=="landscape"){
  mfcol <- c(dim1,dim2)}else{
  mfcol <- c(dim2,dim1)
  }

par(mfcol=mfcol,mgp=mgp,mar=mar,oma=oma,tck=-0.01,xpd=FALSE,xaxs="r",yaxs="r",cex.axis=cex.axis)
...
assessmentYears_i <- apply(do.call(cbind,lapply(out_i@PPD[[assessmentYears_MP]]@Misc,FUN=function(x){x1 <- x$diagnostic; unlist(lapply(x1,FUN=function(xx){xx$Year}))})),1,median)
}else{
assessmentYears_i <- assessmentYears  
}
abline(v=assessmentYears_i,lty="1111",lwd=2)
}
    if(plot_type=="sims"){
      ...
      grid(nx=NA,ny=NULL)
      abline(v=assessmentYears_i,lty=2,lwd=1)
      }else{
...
if(is.null(assessmentYears)){
assessmentYears_i <- apply(do.call(cbind,lapply(out_i@PPD[[MPNamesSub]]@Misc,FUN=function(x){x1 <- x$diagnostic; unlist(lapply(x1,FUN=function(xx){xx$Year}))})),1,median)
}else{
  assessmentYears_i <- assessmentYears
}
...
points(apply(Fstatus,2,proyear_phase_FUN),
       apply(Bstatus,2,proyear_phase_FUN),pch=21,cex=3,lwd=2,col="black",bg=makeOpaque(cols))
...
if(i%in%topPanels&i%in%leftPanels){
  ...
  legend("topright",legend=leg_text_phase,bty="n",text.col="black",text.font = 2,x.intersp = legend_x.intersp)
}  
...
if(textlabpos[i]=="lo"){text(x=x,y=rep(par("usr")[3]),labels=Statmedtext,srt=90,adj=c(-0.1,0.5),font=1)}
if(textlabpos[i]=="up"){text(x=x,y=rep(par("usr")[4]),labels=Statmedtext,srt=90,adj=c(1.2,0.5),font=1)}
...
PMsummary <- function(PMName,
                      ...
                      par_args = list(mfrow = c(length(OMNames),1), mar = c(0,2,0,1), 
                                      oma = c(3,3,1,0), mgp = c(0.8,0.2,0), tck = -0.01, cex.axis=1.0),
                      y.cex.axis=1.2,
  ...
  if(!"outcol"%in%names(dots)){
    dots$outcol <- dots$col
  }
  if(!"outpch"%in%names(dots)){
    dots$outpch <- 16
  }
  ...
  for (OMNames_i in OMNames){
    ...
    for(MSEoutNames_ij in MSEoutNames_i){
      ...
      for(k in 1:n_yr_groups){
        ...
        dimnames(out_ijk) <- list("sim"=1:dim(out_ijk)[1], "MPs"=MPs_ijk)
  ...
  xaxis_label_at <- apply(matrix(box_at,nrow=n_group,ncol=length(xaxis_label)),2,mean)
  
  axis(side = 1, at = xaxis_label_at, labels = xaxis_label)
  mtext(xaxis_label_outer,side=1,line=1.5, outer=TRUE,font=1)
...
MSEsummary <- function(MSEslotName,
                      ...
                      par_args=list(mfrow=c(length(OMNames),1), 
                                    mar=c(0,2,0,1), oma=c(3,3,1,0),
                                    mgp=c(0.8,0.2,0), tck=-0.01, cex.axis=1.0),
                      y.cex.axis=1.2,
                      n_yr_groups=1, # Number of groups to split years into
                      FUN_yr_groups=NULL,
                      #scale_by_nsim=FALSE,
...
for (OMNames_i in OMNames){
  ...
  for(MSEoutNames_ij in MSEoutNames_i){
    ...
    for(m in 1:n_yr_groups){
      ...
      # if(scale_by_nsim){
      #   slot_km <- slot_km/nsim_ij
      # }
      }
    if(!is.null(MPs)){
      slot_km <- slot_km[,match(MPs,MPs_ijm)]
      MPs_ijm <- MPs
    }
      dimnames(slot_km) <- list("sim"=1:dim(slot_km)[1], "MPs"=MPs_ijm)
...
```{r data}
# Read in list of rdat modifications used when plotting bam results compared with Hist objects
load("rdatmods.RData")
...
file_OMNames_base <- list.files("OM_modified")[grepl("base.rds$",list.files("OM_modified"))]
OMNames_base <- gsub(".rds$","",file_OMNames_base)
for(file.i in file_OMNames_base){
...
for(file.i in list.files("MSE_obj")){
  if(file.i%in%paste0("MSE_",rep(Names_user,each=length(sN)),"_",sN,".rds")){
...
MSEoutNames_user1 <- paste0(rep(gsub("OM","MSE",OMNames),each=length(get(sN_run[1]))),
                     "_",
                     rep(get(sN_run[1]),length(OMNames))
                     )

```

```{r OM summary table}
# Get values from base model OMs
for(OMNames_i in OMNames_user){
MSE_i <- get(paste0(gsub("^OM_","MSE_",OMNames_i),"_base"))
MSEOM_i <- MSE_i@OM
steep <- range(MSEOM_i$hs)
Linf <- range(MSEOM_i$Linf)
}
...
for(MP_name_i in "SCA_5"){#MPNamesAll[1]){
    # Ind
    plot_MSE_ts(
      OMNames= OMNames, scenNames=get(sN[1]),
      myvarName = expression(slot(out_i@PPD[[MP_name_i]],"Ind")),
      plot_type = "boot",
      FUN = mean_noNA,
      ylabel = "index of abundance",
      MPNamesSub = MPNamesSub_userA,
      MPNamesSub_legtext=MPNamesSub_userA_legtext,
      rowLabs=scenNames_key[get(sN[1])],
      oma=c(3,4,5,3),
      ylim=c(0,4)
    )
...
for(MP_name_i in "SCA_10"){#MPNamesAll){
    ...
    # MSE_i <- get(MSE_name_j)
    plot_MSE_ts(
      OMNames= OMNames, scenNames="ucvlo",
      myvarName = expression(slot(out_i@PPD[[MP_name_i]],"AddInd")),
      plot_type = "addind",
      ylabel = "index of abundance",
      # FUN=mean_noNA,
...
```{r plot sims}
MPs_sim <- c("SCA_1","SCA_10","iMP_avg_10","pMP_10")
scen <- c("base")

for(scen_i in scen){
for(OMName_i in OMNames_user){
Name_i <- gsub("^OM_","",OMName_i)
MSEoutName_i <- paste0(gsub("^OM","MSE",OMName_i),paste0("_",scen_i))
...
plot_MSE_sims(MSEOut_i,MPs=MPs_sim,type="histproj",
              ...
              title_main = paste0(Name_i,", ",scen_i))
if(plot_pdf){dev.off()}
}
}  
...
```{r boxplots plots combining multiple scenarios, eval=TRUE}
# Summary plots combining all years and all scenarios
for(i in match(sN_plot,sN_all)){
  
sN_i <- get(sN_all[i])
  
if(plot_pdf){pdf(paste0("Figs/boxplot","PBgrt75BMSY",i,".pdf"))}
P75_out <- PMsummary("P75",scenNames=sN_i,MPs=MPNamesAll,
                     las=2,outline=boxplot_outline,range=boxplot_range,
                     boxwex=0.5,ylab="",ylim=c(0,1.1),
                     hline=NA,n_yr_groups=3,
                     xaxis_label=MPNamesAll_abb
                     )       
mtext("P(B > 0.75 BMSY)",side = 2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","PBgrtBMSY",i,".pdf"))}
P100_out <- PMsummary("P100",scenNames=sN_i,MPs=MPNamesAll,
                     las=2,outline=boxplot_outline,range=boxplot_range,
                     boxwex=0.5,ylab="",ylim=c(0,1.1),
                     hline=NA,n_yr_groups=3,
                     xaxis_label=MPNamesAll_abb
                     )
mtext("P(B > BMSY)",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","PFlssFMSY",i,".pdf"))}
PNOF_out <- PMsummary("PNOF",scenNames=sN_i,MPs=MPNamesAll,
                     las=2,outline=boxplot_outline,range=boxplot_range,
                     boxwex=0.5,ylab="",ylim=c(0,1.1),
                     hline=NA,n_yr_groups=3,
                     xaxis_label=MPNamesAll_abb
                     )
mtext("P(F < FMSY)",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","AAVY",i,".pdf"))}
AAVY_out <- PMsummary("AAVY",OMNames = OMNames_user,
                      scenNames=sN_i,MPs=MPNamesAll,
                     slot_to_plot = "Stat",
                     las=2,outline=boxplot_outline,range=boxplot_range,
                     boxwex=0.5,ylab="",ylim=c(0,1.0),
                     hline=0.2,n_yr_groups=3,
                     xaxis_label=MPNamesAll_abb
                     )   
mtext("Avg. Annual Variability in Yield",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","SBSBMSY",i,".pdf"))}
MSEsummary("SB_SBMSY",scenNames=sN_i,MPs=MPNamesAll,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,hline=1,ylim=c(0,5),
           xaxis_label=MPNamesAll_abb
)
mtext("SSB/SSBmsy",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","FFMSY",i,".pdf"))}
MSEsummary("F_FMSY",scenNames=sN_i,MPs=MPNamesAll,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,hline=1,ylim=c(0,5),
           xaxis_label=MPNamesAll_abb
)
mtext("F/Fmsy",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","CatchTotal",i,".pdf"))}
MSEsummary("Catch",scenNames=sN_i,MPs=MPNamesAll,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,FUN_yr_groups=sum,
           xaxis_label=MPNamesAll_abb
)
mtext("Total catch (1000 lb)",2,outer=TRUE,line=1.2,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","CatchTotalMSY",i,".pdf"))}
MSEsummary(expression(MSEout_ij@Catch/
                        (MSEout_ij@RefPoint$MSY[,,-(1:MSEout_ij@nyears)]*MSE_BlackSeaBass_base@proyears)),
           scenNames=sN_i,MPs=MPNamesAll,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,FUN_yr_groups=sum,
           xaxis_label=MPNamesAll_abb,
...
mtext("Total catch/(MSY*proyears)",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

# My version of AAVY, to compare with the openMSE function used in PMsummary
if(plot_pdf){pdf(paste0("Figs/boxplot","AAVYNK",i,".pdf"))}
AAVYNK_out <- MSEsummary("Catch",scenNames=sN_i,MPs=MPNamesAll,
                         OMNames = OMNames_user,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,FUN_yr_groups=function(x){mean(ts_reldiff(x,lag=1))},
           xaxis_label=MPNamesAll_abb,hline=0.2,ylim=c(0,1)
)
mtext("Avg. Annual Variability in Yield",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

# My version of AAVY with a lag of 2
if(plot_pdf){pdf(paste0("Figs/boxplot","AAVYNKlag2yr",i,".pdf"))}
AAVYNK_out <- MSEsummary("Catch",scenNames=sN_i,MPs=MPNamesAll,
                         OMNames = OMNames_user,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,FUN_yr_groups=function(x){mean(ts_reldiff(x,lag=2))},
           xaxis_label=MPNamesAll_abb,hline=0.2,ylim=c(0,1)
)
mtext("Avg. Annual Variability in Yield (lag 2 yr)",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

# 75th percentile of interannual variation in catch
if(plot_pdf){pdf(paste0("Figs/boxplot","75pctAVY",i,".pdf"))}
boxplot75pctAVY1_out <- MSEsummary("Catch",scenNames=sN_i,MPs=MPNamesAll,
                         OMNames = OMNames_user,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,
           FUN_yr_groups=function(x){as.numeric(quantile(ts_reldiff(x,lag=1),probs=0.75))},
           xaxis_label=MPNamesAll_abb,hline=0.2,ylim=c(0,1)
)
mtext("75th percentile Annual Variability in Yield",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/boxplot","CatchCV",i,".pdf"))}
MSEsummary("Catch",scenNames=sN_i,MPs=MPNamesAll,
           las=2,outline=boxplot_outline,range=boxplot_range,boxwex=0.5,ylab="",
           n_yr_groups=3,FUN_yr_groups=cv,hline=NA,
           xaxis_label=MPNamesAll_abb,
                            ylim=c(0,2)
)
mtext("cv catch",2,outer=TRUE,font=1)
if(plot_pdf){dev.off()}

}
```

```{r plot time series A,eval=TRUE}
legend_inset_1 <- c(0,-0.5)
legend_inset <- c(0,-0.8)
...
for(i in match(sN_plot,sN_all)){
if(i==1){
  legend_inset_i<-legend_inset_1
  pdf_hgt_i <- pdf_hgt_1
}else{
  legend_inset_i<-legend_inset
  pdf_hgt_i <- pdf_hgt
}  
...
if(length(sN_i)==1){
  add_args <- list(plot_format = "portrait",
                   main_line = -1.5,
                   legend_panel = 4,
                   mgp=c(1,0.2,0),
                   legend_cex=1.2,
                   legend_inset=legend_inset_i)
}else{
  add_args <- list(legend_cex=1.2,
                   legend_inset=legend_inset_i)
}
...
# do.call(plot_MSE_ts,
#  c(list(
# OMNames= OMNames, scenNames=sN_i,
# myvarName = "VB",
# ylabel = expression(median~(VB)),
# FUN = FUN_tmp,
# MPNamesSub = MPNamesSub_userA,
# MPNamesSub_legtext=MPNamesSub_userA_legtext,
# rowLabs=scenNames_key[sN_i]
# ),add_args))

# SSB
if(plot_pdf){pdf(paste0("Figs/ts","SSB",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SSB",
...
# F
if(plot_pdf){pdf(paste0("Figs/ts","F",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
OMNames= OMNames, scenNames=sN_i,
myvarName = "FM",
...
# Catch
if(plot_pdf){pdf(paste0("Figs/ts","Catch",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
OMNames= OMNames, scenNames=sN_i,  
myvarName = "Catch",
...
# SSB/SSBmsy
if(plot_pdf){pdf(paste0("Figs/ts","SSBSSBmsy",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SB_SBMSY",
...
# F/Fmsy
if(plot_pdf){pdf(paste0("Figs/ts","FFmsy",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "F_FMSY",
...
# C/MSY
if(plot_pdf){pdf(paste0("Figs/ts","CatchMSY",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "Catch",
...
#   myvarName = "TAC",
#   myvarDenomExpr = expression(slot(out_i,"RefPoint")$MSY[,,-(1:out_i@nyears)]),
#   ylabel = expression(median~(TAC/MSY)),
#   FUN = FUN_tmp,
#   ylim=c(0,2),
#   MPNamesSub = MPNamesSub_userA,
#   MPNamesSub_legtext=MPNamesSub_userA_legtext,
...
# FUN = FUN_tmp,
# ylabel= expression(median~(SSBMSY)),
# MPNamesSub = MPNamesSub_userA,
# MPNamesSub_legtext=MPNamesSub_userA_legtext,
# rowLabs=scenNames_key[sN_i]
# ),add_args))
...
```{r plot time series B,eval=TRUE}
legend_inset_1 <- c(0,-0.5)
legend_inset <- c(0,-0.8)
...
# ylabel = expression(median~(VB)),
# FUN = FUN_tmp,
# MPNamesSub = MPNamesSub_userB,
# MPNamesSub_legtext=MPNamesSub_userB_legtext,
# rowLabs=scenNames_key[sN_i],
#  assessmentYears_MP="SCA_5"
# ),add_args))

# SSB
if(plot_pdf){pdf(paste0("Figs/ts","SSB",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SSB",
...
# F
if(plot_pdf){pdf(paste0("Figs/ts","F",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
OMNames= OMNames, scenNames=sN_i,
myvarName = "FM",
...
# Catch
if(plot_pdf){pdf(paste0("Figs/ts","Catch",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
OMNames= OMNames, scenNames=sN_i,  
myvarName = "Catch",
...
# SSB/SSBmsy
if(plot_pdf){pdf(paste0("Figs/ts","SSBSSBmsy",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SB_SBMSY",
...
# F/Fmsy
if(plot_pdf){pdf(paste0("Figs/ts","FFmsy",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "F_FMSY",
...
# C/MSY
if(plot_pdf){pdf(paste0("Figs/ts","CatchMSY",i,"B.pdf"), width=pdf_wid, height=pdf_hgt_i)}
do.call(plot_MSE_ts,
  c(list(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "Catch",
...
# SSB
if(plot_pdf){pdf(paste0("Figs/tsRE","SSB",i,".pdf"))}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SSB",
...
# Catch
if(plot_pdf){pdf(paste0("Figs/tsRE","Catch",i,".pdf"))}
...
# SSB/SSBmsy
if(plot_pdf){pdf(paste0("Figs/tsRE","SSBSSBMSY",i,".pdf"))}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  myvarName = "SB_SBMSY",
...
# F/Fmsy
if(plot_pdf){pdf(paste0("Figs/tsRE","FFMSY",i,".pdf"))}
...
if(plot_pdf){pdf(paste0("Figs/tsRE","CatchMSY",i,".pdf"))}
...
if(plot_pdf){pdf(paste0("Figs/dw","SSB",i,".pdf"))}
plot_MSE_dw(
    OMNames= OMNames, scenNames=sN_i,
    myvarName = "SSB",
...
if(plot_pdf){pdf(paste0("Figs/dw","Catch",i,".pdf"))}
plot_MSE_dw(
    OMNames= OMNames, scenNames=sN_i,
    myvarName = "Catch",
    myvarDenomExpr = expression(slot(out_i,"RefPoint")$MSY[,,-(1:out_i@nyears)]),
...
legend_inset_1 <- c(0.7,-0.27)
legend_inset <- c(0.7,-0.7)
legend_cex <- 1.2
legend_ncol <- ceiling(length(MPNamesAll)/2)
legend_horiz <- FALSE
legend_x <- "bottomleft"
legend_xjust <- 0.5
...
if(plot_pdf){pdf(paste0("Figs/tr","YldP100",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "tradeoff",
  PMx = Yield,
  PMy = P100,
  MPNamesSub = MPNamesAll,
  MPNamesSub_legtext=MPNamesAll_abb,
  rowLabs=scenNames_key[sN_i],
  mar=c(0,0,0,0),label_axes="leftbottom",
  xlim=c(0,1.2),ylim=c(0,1.1),
  hline=0.5,vline=0.5,cex=2,legend_cex = legend_cex, legend_x.intersp=0.1,
  legend_inset=legend_inset_i,
  legend_horiz=legend_horiz,
  legend_ncol=legend_ncol,
  legend_x=legend_x,
  legend_xjust=legend_xjust,
  cex.axis=cex.axis
)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf(paste0("Figs/tr","PNOFP100",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "tradeoff",
  PMx = PNOF,
...
if(plot_pdf){pdf(paste0("Figs/tr","AAVYP100",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames,
  scenNames = sN_i,
  plot_type = "tradeoff",
...
if(plot_pdf){pdf(paste0("Figs/tr","AAVYPNOF",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "tradeoff",
  PMx = AAVY,
...
if(plot_pdf){pdf(paste0("Figs/tr","AAVYYldLT",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "tradeoff",
  PMx = AAVY,
...
if(plot_pdf){pdf(paste0("Figs/tr","P100YldLT",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "tradeoff",
  PMx = P100,
...
if(plot_pdf){pdf(paste0("Figs/phasePlot",i,".pdf"), width=pdf_wid, height=pdf_hgt_i)}
plot_MSE_ts(
  OMNames= OMNames, scenNames=sN_i,
  plot_type = "phase",
  proyear_phase=41:50,
  MPNamesSub = c(MPNamesSub_userA),
  MPNamesSub_legtext=c(MPNamesSub_userA_legtext),
  rowLabs=scenNames_key[sN_i],
  mar=c(0,0,0,0),label_axes="leftbottom",
...
```{r generate Hist, eval=TRUE}
if(generate_Hist){
  
for(OMName_base_i in OMNames_base){
  OM_base_i <- get(OMName_base_i)
  
  cat(paste0("\n\nWorking on ",OMName_base_i,"\n"))
  Hist_i <- MSEtool::Simulate(OM_base_i) # Generate historical data

  if(OMName_i%in%OMNames){
  Hist_name_i <- gsub("OM","Hist",OMName_base_i)
  assign(Hist_name_i,Hist_i)
  saveRDS(Hist_i, file = file.path("Hist",paste0(Hist_name_i,".rds")))
  }
...
for(OMName_base_i in OMNames_base){
  Hist_name_i <- gsub("OM","Hist",OMName_base_i)
  Hist_i <- readRDS(file = file.path("Hist",paste0(Hist_name_i,".rds")))
  assign(Hist_name_i,Hist_i)
}
}
```

```{r plot Hist with bam  results, eval=FALSE}
if(plot_Hist){
CIpct  <-  95
...
if(plot_pdf){pdf(paste0("Figs/Hist_",i,"_base.pdf"))}  
rdat_name_i <- paste0("rdat_",i)
  
OM_base_i <- get(paste0("OM_",i,"_base"))
# Data_i <- get(paste0("Data_",i))
Hist_i <- get(paste0("Hist_",i,"_base"))
...
bam.U.ob <- t.series_i[,U.names.ob[which(!is.na(tail(t.series_i[,U.names.ob],1)))],drop=FALSE]
for(j in 1:nAddInd){
