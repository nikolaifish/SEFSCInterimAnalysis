...
scenNames_user1 <-  c("base","dep","lf","epiM","ubias","tachi","rc")
scenNames_user2 <-  c("base","nolag","minerr","minrecdev","perfobs","ucvhi","ucvlo","vtiv"
                        )

# All scenarios
scenNames_user <- unique(c(scenNames_user1,scenNames_user2))
...
scenNames_user_alt <- c("base","dep","lf","epiM","ubias","tachi","rc")
...
MPNamesAll_key <- c("AvC"="average catch",
                    ...
                    "pMP_5" = "projection (5 yr)",
                    "pMP_10" = "projection (10 yr)"
                        )
...
PMsummary <- function(PMName,OMNames=OMNames_user,scenNames,MPs=NULL,
                      slot_to_plot="Prob", # Can be either "Prob" or "Stat"
                      main_line=-1.5,
                      hline=1,hline_lty=1,
                      par_args=list(mfrow=c(length(OMNames),1), 
                                    mar=c(0,2,0,1), oma=c(6,2,1,0),
...
MSEoutNames_i = paste0(rep(gsub("OM","MSE",OMNames_i),each=length(scenNames)),
    ...
    for(MSEoutNames_ij in MSEoutNames_i){
      ...
      PMout_ij <- PM(MSEout_ij)
        MPs_ij <- PMout_ij@MPs
        out_ij <- slot(PMout_ij,slot_to_plot)
      if(!is.null(MPs)){
        out_ij <- out_ij[,match(MPs,MPs_ij)]
        MPs_ij <- MPs
      }
      dimnames(out_ij) <- list("sim"=seq(1:dim(out_ij)[1]), "MPs"=MPs_ij)
      out_ij <- cbind("scen"=scenNames_j,as.data.frame(out_ij))
      out_list_i[[MSEoutNames_ij]] <- out_ij
    ...
    # Plot stuff
      dots$xaxt <- ifelse(match(OMNames_i,rev(OMNames))==1,"s","n")
      
      do.call(boxplot,c(list(out_df_i[,-1],main=""),dots))
...
# compute and plot summary statistics for Performance metrics
MSEsummary <- function(MSEslotName,OMNames=OMNames_user,scenNames,MPs=NULL,
                       main_line=-1.5,
                       hline=1,hline_lty=1,
                      par_args=list(mfrow=c(length(OMNames),1),
                                    mar=c(3,2,1,1), oma=c(1,2,0,0),
                                    mgp=c(0.8,0.2,0), tck=-0.01),
                      ...){
dots <- list(...)
if(!"ylab"%in%names(dots)){
...
for (OMNames_i in OMNames){
  ...
  for(MSEoutNames_ij in MSEoutNames_i){
    ...
    if(!is.expression(MSEslotName)){
      slot_k <- slot(MSEout_ij,MSEslotName) %>% apply(2,as.numeric)
    }else{
      slot_k <- eval(MSEslotName) %>% apply(2,as.numeric)
    }
    slot_k <- slot(MSEout_ij,MSEslotName) %>% apply(2,as.numeric)
    MPs_ij <- MSEout_ij@MPs
    if(!is.null(MPs)){
      slot_k <- slot_k[,match(MPs,MPs_ij)]
      MPs_ij <- MPs
    }
    dimnames(slot_k) <- list(NULL, "MPs"=MPs_ij)
    slot_k <- cbind("scen"=scenNames_j,as.data.frame(slot_k))
    slot_list_i[[MSEoutNames_ij]] <- slot_k
  ...
  # Plot stuff
  do.call(boxplot,c(list(slot_df_i[,-1],main=""),dots))
  mtext(Names_i,side=3,line=main_line,outer=FALSE,font=2)
  abline(h=hline,lty=hline_lty)
}
...
for(MP_name_i in "SCA_5"){#MPNamesAll){
    # Ind
...
MPs_sim <- c("SCA_1","SCA_5","iMP_avg_10")
scen <- "base"
...
figName_ij <- paste0("sims_",MSEoutName_i,"_",scen,".pdf")
...
if(plot_pdf){pdf("Figs/boxplot_P_Bgrt75BMSY.pdf")}
P75_out <- PMsummary("P75",scenNames=scenNames_user_alt,MPs=MPNamesAll,
                     cex.axis=1,las=2,outline=FALSE,
                     boxwex=0.25,ylab="",ylim=c(0,1.1),
                     hline=NA)
mtext("P(B > 0.75 BMSY)",2,outer=TRUE,font=2)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf("Figs/boxplot_P_BgrtBMSY.pdf")}
P100_out <- PMsummary("P100",scenNames=scenNames_user_alt,MPs=MPNamesAll,
                     cex.axis=1,las=2,outline=FALSE,
                     boxwex=0.25,ylab="",ylim=c(0,1.1),
                     hline=NA)
...
if(plot_pdf){pdf("Figs/boxplot_P_FlssFMSY.pdf")}
PNOF_out <- PMsummary("PNOF",scenNames=scenNames_user_alt,MPs=MPNamesAll,
                     cex.axis=1,las=2,outline=FALSE,
                     boxwex=0.25,ylab="",ylim=c(0,1.1),
                     hline=NA)
...
if(plot_pdf){pdf("Figs/boxplot_AAVY.pdf")}
AAVY_out <- PMsummary("AAVY",scenNames=scenNames_user_alt,MPs=MPNamesAll,
                      slot_to_plot = "Stat",
                     cex.axis=1,las=2,outline=FALSE,
                     boxwex=0.25,ylab="",ylim=c(0,1.1),
                     hline=NA)
...
if(plot_pdf){pdf("Figs/boxplot_SB_SBMSY.pdf")}
MSEsummary("SB_SBMSY",scenNames=scenNames_user_alt,MPs=MPNamesAll,
           cex.axis=0.75,las=2,outline=FALSE,boxwex=0.25,ylab="")
mtext("SSB/SSBmsy",2,outer=TRUE,font=2)
if(plot_pdf){dev.off()}

if(plot_pdf){pdf("Figs/boxplot_F_FSY.pdf")}
MSEsummary("F_FMSY",scenNames=scenNames_user_alt,MPs=MPNamesAll,
           cex.axis=0.75,las=2,outline=FALSE,boxwex=0.25,ylab="")
...
```{r plot time series1,eval=FALSE}
# # VB
# plot_MSE_ts(
# OMNames= OMNames, scenNames=scenNames_user1,
# myvarName = "VB",
...
# SSB
if(plot_pdf){pdf("Figs/ts_SSB.pdf")}
plot_MSE_ts(
  OMNames= OMNames, scenNames=scenNames_user1,
  myvarName = "SSB",
...
# Catch
if(plot_pdf){pdf("Figs/ts_Catch.pdf")}
plot_MSE_ts(
OMNames= OMNames, scenNames=scenNames_user1,  
myvarName = "Catch",
...
# SSB/SSBmsy
if(plot_pdf){pdf("Figs/ts_SSB_SSBmsy.pdf")}
plot_MSE_ts(
  OMNames= OMNames, scenNames=scenNames_user1,
  myvarName = "SB_SBMSY",
...
# F/Fmsy
if(plot_pdf){pdf("Figs/ts_F_Fmsy.pdf")}
plot_MSE_ts(
  OMNames= OMNames, scenNames=scenNames_user1,
  myvarName = "F_FMSY",
...
# C/MSY
if(plot_pdf){pdf("Figs/ts_Catch_MSY.pdf")}
plot_MSE_ts(
  OMNames= OMNames, scenNames=scenNames_user1,
  myvarName = "Catch",
...
```{r tradeoffPlot1, eval=FALSE}
if(plot_pdf){pdf("Figs/tradeoffPlot1.pdf")}
plot_MSE_ts(
  OMNames= OMNames, scenNames=scenNames_user1,
  # MPNamesRef = "SCA_1",
  plot_type = "tradeoff",
