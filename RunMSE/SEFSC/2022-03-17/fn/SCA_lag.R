SCA_lag <- function (x = 1, Data, lag=0,
                     AddInd = "B", AddInd_all=TRUE,
                     SR = c("BH", "Ricker", "none"),
                     vulnerability = c("logistic", "dome"),
                     catch_eq = c("Baranov", "Pope"),
                     CAA_dist = c("multinomial", "lognormal"), CAA_multiplier = 50,
                     rescale = "mean1", max_age = Data@MaxAge, start = NULL,
                     prior = list(), fix_h = TRUE, fix_F_equilibrium = TRUE,
                     fix_omega = TRUE, fix_tau = TRUE, LWT = list(),
                     ...
                     inner.control = list(), ...)
{
  ...
  Data <- bamMSE::Data_sub_dim(Data,nyr_rm=lag) # Trim temporally explicit slots based on lag

  # Override AddInd setting (which is also used by interim procedures) to use all available indices
  # in the assessments.
  if(AddInd_all){
    if(any(!is.na(as.numeric(Data@AddInd)))){
      AddInd <- 1:dim(Data@AddInd)[2]}
  }

  out <- SAMtool:::SCA_(x = x, Data = Data, AddInd = AddInd, SR = SR,
