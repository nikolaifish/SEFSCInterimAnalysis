---
title: "Application of Huynh 2020 Interim Analysis methods to other stocks: investigating if stocks tend toward MSY with HCR_MSY harvest control rule."
...
OMNames_user <- c("MSE_BlackSeaBass","MSE_VermilionSnapper","MSE_RedPorgy")#,
#                    #"OM_Tilefish","OM_Cobia","OM_SnowyGrouper",
#                    #"OM_capelin", "OM_POP", "OM_vs",
#   "OM_Albacore","OM_AlmacoJack", "OM_BandedMorwong"
#                    )
# OMNames_user <- local({
...
OMNames_user <- gsub("MSE","OM",OMNames_user)
scenNames_user <-  c("base","nr"#,#"hs","hd",
                     #"dep","lf",
                     #"epiM"
                     )
OMscenNames_user <- sort(apply(expand.grid(OMNames_user,scenNames_user),1,function(x){paste(x,collapse="_")}))
scenNames_key <- c("base"="Base","nr"="no rec error","hs"="Hyper-stable","hd"="Hyper-deplete",
                   "dep"="Depleted","lf"="Lightly fished","epiM"="Episodic M")
MPNamesSub_user <- c("SCA_1","SCA_10","iMP_avg_10","MP_PerfectMSY")#,"iMP_avg_10","iMP_buffer_10")
MPNamesSub_user_legtext <- c("Annual assessment","Fixed TAC (10)","Averaged Index (10)","Perfect MSY")#,"Buffered Index (10)")
```

```{r plot_LH function, eval=TRUE}
plot_LH <- function(OM,...){
maxage <- OM@maxage
agebins <- 0:maxage
...
legtext_o = c(1,2,3,4), # order for plotting legend text
...
mar=c(1,1,1,1),
cols = setNames(c("black","red","deepskyblue2","green3"),MPNamesSub),
ltys = setNames(c(1,2,1,2),MPNamesSub),
...
if(is.null(colLabs)){
  colLabs <- local({
  a <- gsub("OM_","",OMNames) # Replace OM_
  b <- str_replace_all(a,"_[A-Za-z]*$","") # Replace underscore and letters following
  c <- str_replace_all(b,"(?<=[a-z])(?=[A-Z])"," ")
  unique(c)
  })
}  
...
myvarsub <- myvar[,match(MPNamesSub,MPNames),,drop=FALSE]
...
```{r plot_results function}
plot_results <- function(OMNames,plots=c("SSB/SSBmsy","F/Fmsy","Catch/MSY","TAC/MSY")){
  speciesNames <- gsub("OM_","",OMNames)
  
  # SSB/SSBmsy
  if("SSB/SSBmsy"%in%plots){    
    plot_MSE_ts_Huynh(
  ...
  # Catch/TAC
  if("Catch/TAC"%in%plots){ 
    plot_MSE_ts_Huynh(
      OMNames= OMNames, scenNames=scenNames_user,
      myvarName = "Catch",
      myvarDenomExpr = expression(slot(out_i,"TAC")),
      ylabel = expression(mean~~Catch/TAC),
      FUN = mean,
      ylim=c(0,1.1),
      MPNamesSub = MPNamesSub_user,
      MPNamesSub_legtext=MPNamesSub_user_legtext,
  ...
  if("MSY"%in%plots){     
    plot_MSE_ts_Huynh(
      OMNames= OMNames, scenNames=scenNames_user,
      myvarName = expression(slot(out_i,"RefPoint")$MSY[,,-(1:out_i@nyears),drop=FALSE]),
      FUN = mean,
      ylabel="MSY",
      MPNamesSub = MPNamesSub_user,
      MPNamesSub_legtext=MPNamesSub_user_legtext,
  ...
  if("SSBMSY"%in%plots){ 
    plot_MSE_ts_Huynh(
      OMNames= OMNames, scenNames=scenNames_user,
      myvarName = expression(slot(out_i,"RefPoint")$SSBMSY[,,-(1:out_i@nyears),drop=FALSE]),
      FUN = mean,
      ylabel="SSBMSY",
      MPNamesSub = MPNamesSub_user,
      MPNamesSub_legtext=MPNamesSub_user_legtext,
  ...
  if("FMSY"%in%plots){     
    plot_MSE_ts_Huynh(
      OMNames= OMNames, scenNames=scenNames_user,
      myvarName = expression(slot(out_i,"RefPoint")$FMSY[,,-(1:out_i@nyears),drop=FALSE]),
      FUN = mean,
      ylabel="FMSY",
      MPNamesSub = MPNamesSub_user,
      MPNamesSub_legtext=MPNamesSub_user_legtext,
  ...
  if("UMSY"%in%plots){     
    plot_MSE_ts_Huynh(
      OMNames= OMNames, scenNames=scenNames_user,
      myvarName = expression(F2U(slot(out_i,"RefPoint")$FMSY[,,-(1:out_i@nyears),drop=FALSE])),
      FUN = mean,
      ylabel="UMSY",
      MPNamesSub = MPNamesSub_user,
      MPNamesSub_legtext=MPNamesSub_user_legtext,
...
```{r plot results,eval=TRUE}
# ct <- 1
# while(ct<=length(OMscenNames_user)){
#   OMscenNames_i <- as.character(na.omit(OMscenNames_user[ct+0:2]))
#   plot_results(OMNames=OMscenNames_i)
#   ct <- ct+3
# }

  plot_results(OMNames=OMNames_user)
