SCA__NK <- function (x = 1, Data, AddInd = "B", SR = c("BH", "Ricker",
                                                             ...
                                                             eval.max = 4e+05), inner.control = list(), ...)
{
  ...
  n_y <- length(C_hist)
  if(!is.null(is.null(Data@Misc$M_at_age))){
    M <- Data@Misc$M_at_age
  }else{
  ...
  Ind <- lapply(AddInd, SAMtool:::Assess_I_hist, Data = Data, x = x,
                yind = yind)
  ...
  prior <- SAMtool:::make_prior(prior, nsurvey, ifelse(SR == "BH", 1,
                                             2), msg = FALSE)
  if (is.null(M_bounds)) {
    if (tv_M == "none") {
      M_bounds <- c(0, 10000)
  ...
  data <- list(model = "SCA", C_hist = C_hist, rescale = rescale,
               ...
               abs_I = rep(0, nsurvey), nsurvey = nsurvey, CAA_hist = apply(CAA_hist,
                                                                            1, SAMtool:::tiny_comp) %>% t(), CAA_n = CAA_n_rescale, CAL_hist = apply(CAL_hist,
                                                                                                                                           1, SAMtool:::tiny_comp) %>% t(), CAL_n = CAL_n_rescale, LWT = c(LWT$Index,
  ...
  if (!is.null(start)) {
    ...
    if (!is.null(start$h) && is.numeric(start$h)) {
      if (SR == "BH") {
        ...
        params$transformed_h <- SAMtool:::logit(h_start)
      }
      else if (SR == "Ricker") {
    ...
    if (!is.null(start$vul_par) && is.numeric(start$vul_par)) {
      ...
      if (vulnerability == "logistic") {
        ...
        params$vul_par <- c(SAMtool:::logit(start$vul_par[1]/max_age/0.75),
      ...
      if (vulnerability == "dome") {
        ...
        params$vul_par <- c(SAMtool:::logit(start$vul_par[1]/max_age/0.75),
                            log(start$vul_par[1] - start$vul_par[2]),
                            SAMtool:::logit(1/(max_age - start$vul_par[1])), SAMtool:::logit(start$vul_par[4]))
      }
    }
  ...
  if (is.null(params$vul_par)) {
    ...
    if (is.na(Data@LFC[x]) && is.na(Data@LFS[x]) || Data@LFC[x] >
      ...
      if (vulnerability == "logistic")
        params$vul_par <- c(SAMtool:::logit(comp_mode/max_age/0.75),
                            log(1))
      if (vulnerability == "dome") {
        params$vul_par <- c(SAMtool:::logit(comp_mode/max_age/0.75),
                            log(1), SAMtool:::logit(1/(max_age - comp_mode)), SAMtool:::logit(0.5))
      }
    }
    else {
      ...
      if (vulnerability == "logistic")
        params$vul_par <- c(SAMtool:::logit(Afull/max_age/0.75),
                            log(Afull - A50_vul))
      if (vulnerability == "dome") {
        params$vul_par <- c(SAMtool:::logit(Afull/max_age/0.75),
                            log(Afull - A50_vul), SAMtool:::logit(0.1/(max_age -
                                                               Afull)), SAMtool:::logit(0.5))
      }
    }
  }
  ...
  Dev <- structure(c(rev(report$log_early_rec_dev), report$log_rec_dev),
                   names = YearDev)
  report$dynamic_SSB0 <- SAMtool:::SCA_dynamic_SSB0(obj, data = info$data,
  ...
  if (Assessment@conv) {
    ...
    ref_pt <- SAMtool:::ref_pt_SCA(y = refyear, obj = obj, report = report)
    if (catch_eq == "Baranov") {
    ...
    if (catch_eq == "Baranov") {
      ...
      Assessment@F_FMSY <- structure(report$F/Assessment@FMSY,
                                     names = Year)
    }
    else {
      Assessment@UMSY <- report$UMSY
      Assessment@U_UMSY <- structure(report$U/Assessment@UMSY,
                                     names = Year)
    }
    ...
    Assessment@B_BMSY <- structure(report$B/Assessment@BMSY,
                                   names = Yearplusone)
    Assessment@SSB_SSBMSY <- structure(report$E/Assessment@SSBMSY,
                                       names = Yearplusone)
    Assessment@VB_VBMSY <- structure(report$VB/Assessment@VBMSY,
                                     names = Yearplusone)
    Assessment@Dev <- Dev
    Assessment@SE_Dev <- SE_Dev
    Assessment@TMB_report <- report
    catch_eq_fn <- function(Ftarget) {
      SAMtool:::projection_SCA(Assessment, Ftarget = Ftarget, p_years = 1,
                     p_sim = 1, obs_error = list(array(1, c(1, 1,
                                                            nsurvey)), matrix(1, 1, 1)), process_error = matrix(1,
                                                                                                                1, 1)) %>% slot("Catch") %>% as.vector()
    }
    Assessment@forecast <- list(per_recruit = ref_pt[["per_recruit"]],
