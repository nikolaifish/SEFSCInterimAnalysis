...
```{r}
# create completely empty Data object
Data_empty <- local({
  dat <- new("Data")
  for(slotName_i in slotNames(dat)){
    slot_i <- slot(dat,slotName_i)
    if(length(slot_i)>0&is.numeric(slot_i)){
      slot(dat,slotName_i) <- slot_i*NA
    }
  }
  dat
})
...
```{r build OM Assess2OM, eval=TRUE}
...
if(min(ageinit_i)>0){
 ...
 if(AddInd_endyr_only){
   ...
   AddInd_endyr_ix <- which(!is.na(AddInd[1,,dim(AddInd)[3]])) # Just uses the first sim!!
